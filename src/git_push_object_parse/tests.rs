...
// ----- reference serialisers: the inverse of each parser, used as the
// round-trip oracle above. Production never serialises; it only parses.
...
fn serialize_commit_object(commit: &StagingCommit) -> Vec<u8> {
    let mut out = Vec::new();
...
/// mode for subtrees.
fn serialize_tree_object(tree: &StagingTree) -> Vec<u8> {
    let mut out = Vec::new();
