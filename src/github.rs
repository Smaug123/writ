...
impl MintedToken {
    ...
    /// the raw token string, and the [`PromoteMintAudit`] payload to record
    /// inline on the `git_push_resolution` row. Unlike
    ...
    /// session-scoped `request` / `grant_log` audit chain. The four mint
    /// fields are recorded inline on the resolution row instead.
