...
//! The broker still persists [`PlanBody`] and [`Stage`]; only the
//! *interpretation* (splice the plan into the agent's effective
//! prompt) lives here.
...
/// plan body for runs in this `stage`. Callers consult this to decide
/// whether to load the plan before composing the prompt: loading only
/// when the dispatcher will use the result keeps the two in sync (a
/// previous regression — caught by codex on PR #80 — let a wrapper
/// skip the load for review runs and silently bypass the reviewer
/// composition arm).
...
/// Returns the prompt to feed the LLM given the run's stage and the
/// plan body (when one is available). `(Stage::Execute, Some(body))`
/// composes via [`compose_implementer_prompt`]; `(Stage::Review,
/// Some(body))` composes via [`compose_reviewer_prompt`]; every other
/// combination passes the caller's prompt through unchanged.
/// `Stage::Plan` never reads a plan, and `Stage::Review` without a
/// body only arises if `validate_stage_read_plan_binding` was
/// bypassed — passthrough is the conservative answer there.
///
/// Callers decide whether to load a plan body via
