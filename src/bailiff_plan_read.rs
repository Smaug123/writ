...
//! `docs/plans/2026-05-16-slice-d2-review.md` adds the review read at
//! the third seed-OID. Both read helpers pin the same seed-OID
//! convention the matching writer uses, so a round-trip through the
//! per-plan ref doesn't depend on any registry beyond the deterministic
//! seed bytes.
//!
//! A corresponding `read_plan_note` arrives whenever a consumer first
//! needs it (slice F's `bailiff plan show` is the likely landing
//! point); D1.4 / D2.3 deliberately defer it because nothing in the
//! upcoming slices reads the submission note programmatically yet.

use thiserror::Error;

use crate::bailiff_plan_note::{
    DecisionNote, DecisionNoteParseError, PlanId, ReviewNote, ReviewNoteParseError,
    plan_decision_seed_blob_bytes, plan_notes_ref, plan_review_seed_blob_bytes,
};
