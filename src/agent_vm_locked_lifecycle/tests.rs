...
/// Rewinding a snapshot yields the one it was advanced from, facts and all.
///
/// Checked against the typestate walk rather than against itself: the walk is
/// the sequence, so this pins `previous` to what actually happened rather than
/// to a restatement of its own match.
#[test]
fn rewinding_a_snapshot_yields_the_one_before_it() {
    let snapshots = walk();
    assert_eq!(snapshots[0].previous(), None, "nothing precedes the first");
    for pair in snapshots.windows(2) {
        assert_eq!(
            pair[1].previous().as_ref(),
            Some(&pair[0]),
            "{} rewinds to {}",
            pair[1].phase(),
            pair[0].phase()
