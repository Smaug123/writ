...
usage() {
  cat <<'EOF'
Usage: scripts/prove-agent-vm-codex-proxy.sh

Manual proof harness for daemon-managed Codex inside an agent VM, brokered
through the VM HTTP OpenAI reverse proxy.
...
log() {
  printf '[prove-codex-proxy] %s\n' "$*"
}

die() {
  printf '[prove-codex-proxy] error: %s\n' "$*" >&2
  exit 1
}
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/writ-codex-proxy-proof.XXXXXX")"
chmod 700 "$TMP_DIR"
...
PROOF_TOKEN="ghs_codex_proxy_proof_token"
PROOF_BUNDLE_MARKER="fake-git-bundle-for-codex-proxy-proof"
...
dump_audit_summary() {
  ...
  printf '\n[prove-codex-proxy] ==== audit summary: %s ====\n' "$AUDIT_DB" >&2
  sqlite3 "$AUDIT_DB" \
  ...
  sqlite3 "$AUDIT_DB" \
    "SELECT route, decision, deny_reason, http_status, upstream_status, error
       FROM openai_proxy_request
       LEFT JOIN openai_proxy_outcome USING (request_id);" \
    >&2 || true
}

dump_diagnostics() {
  printf '\n[prove-codex-proxy] diagnostics for failed run under %s\n' "$TMP_DIR" >&2
  ...
  dump_audit_summary
  if [[ -d "$STATE_DIR" ]]; then
    printf '\n[prove-codex-proxy] ==== state records: %s ====\n' "$STATE_DIR" >&2
...
config = {
    # Agent-keyed registry: the session's --agent codex selects this App.
    "github_apps": {
        "codex": {
            "app_id": 42,
            "installation_id": 999,
...
assert_stdout_contains_codex_hello() {
...
with open(path, encoding="utf-8", errors="replace") as f:
    for line in f:
        ...
        # Codex --json emits an "agent_message" event whose payload contains
        # the assistant's text. Schemas vary across versions, so accept any
        # event where the type/role mentions agent/assistant message and a
        # string field starts with "Hello".
        flat = json.dumps(evt)
        if "agent_message" not in flat and "assistant" not in flat:
            continue
        strings = []
        collect_strings(evt, strings)
        for s in strings:
            stripped = s.lstrip()
            if stripped.startswith("Hello"):
                hits.append(s)
                break

if not saw_event:
    print("stdout artifact contains no JSON events", file=sys.stderr)
    sys.exit(2)
if not hits:
    print(
        "no agent/assistant message event with text starting with Hello",
        file=sys.stderr,
    )
    sys.exit(1)
print(hits[0])
PY
  then
    dump_log "agent stdout artifact" "$STDOUT_PATH"
    dump_log "agent stderr artifact" "$STDERR_PATH"
    die "stdout artifact does not contain a Codex agent_message event starting with Hello"
  fi
}

assert_openai_proxy_audit() {
  local proxy_count
  proxy_count="$(sqlite3 "$AUDIT_DB" \
    "SELECT COUNT(*)
       FROM openai_proxy_request r
       JOIN openai_proxy_outcome o USING (request_id)
      WHERE r.session_id = '${SESSION_ID}'
        AND r.route = 'responses'
        AND r.decision = 'allow'
        AND o.upstream_status BETWEEN 200 AND 299;")"
  [[ "$proxy_count" -gt 0 ]] || \
    die "audit log does not show a successful proxied OpenAI /v1/responses request"
  ...
  denied_count="$(sqlite3 "$AUDIT_DB" \
    "SELECT COUNT(*)
      ...
      WHERE session_id = '${SESSION_ID}'
        AND decision = 'deny';")"
  [[ "$denied_count" -eq 0 ]] || \
    die "audit log shows denied OpenAI proxy requests for session ${SESSION_ID}"
}
...
log "starting writd with fake GitHub API on port ${FAKE_GITHUB_PORT}, fake Git origin on port ${FAKE_GIT_ORIGIN_PORT}, and OpenAI upstream ${OPENAI_UPSTREAM}"
env -u OPENAI_API_KEY -u WRIT_PROVE_OPENAI_API_KEY \
...
log "Codex proxy proof succeeded for ${IPV4_CIDR}; stdout contains a Codex agent_message starting with Hello, OpenAI proxy audit rows show a successful brokered /v1/responses call, and daemon stop cleanup was verified"
