...
fn every_attempt_contributes_its_own_output() {
    ...
    assert_eq!(
        dossier.plan_body.as_ref().unwrap().as_ref().unwrap().stdout,
        b"# Plan\n\nDo it.\n",
...
fn a_plan_with_no_attempts_still_yields_its_plan_body() {
    ...
    assert_eq!(
        dossier.plan_body.unwrap().unwrap().stdout,
        b"# Plan\n\nNothing yet.\n",
    );
}
...
/// A **truncated** plan body carries its marker rather than rendering
/// as a complete verified plan.
///
/// `read_plan_body_bytes` refuses a truncated planner stdout outright,
/// on the grounds that a captured prefix would mislead the agent that
...
fn a_truncated_plan_body_keeps_its_marker() {
    ...
    // A planner envelope whose stdout writ marked as truncated.
    let output = OutputEnvelope {
        stdout: b"# Plan\n\nDo it, but I ran ou".to_vec(),
        stderr: Vec::new(),
        stdout_truncated_at: Some(27),
        stderr_truncated_at: None,
    };
    let output_bytes = output.to_bytes();
    let metadata = SignedRunMetadata {
    ...
    .expect("planting a truncated submission");
    ...
    let body = dossier
        .plan_body
        .unwrap()
        .expect("it verifies; it is merely short");
    assert_eq!(
        body.stdout_truncated_at,
        Some(27),
        "the truncation marker must survive into the dossier",
    );
}
