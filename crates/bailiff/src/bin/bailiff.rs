...
enum PlanCmd {
    ...
    Dossier {
        ...
        #[arg(long)]
        writ_allowed_signers: PathBuf,
        /// Write agent bytes to a terminal anyway.
        ///
        /// Without this the verb refuses when stdout is a TTY. The
        /// output embeds implementer stdout verbatim, and an
        /// implementer controls those bytes: ANSI CSI/OSC sequences can
        /// erase or rewrite lines already on screen, including the
        /// `verification=` header that says whether the output beneath
        /// it can be trusted. Length-prefixing keeps a *parser* safe;
        /// it does nothing for a terminal, which interprets rather than
        /// counts.
        ///
        /// Redirecting to a file or a pager is the normal answer; this
        /// flag exists for an operator who has decided the risk is
        /// theirs to take.
        #[arg(long)]
        allow_terminal: bool,
    },
...
async fn plan_dossier(
    ...
    // Refuse before doing any work, not after: the point is that the
    // operator never sees agent-controlled bytes on a terminal, and a
    // check after the git work would still have produced them.
    //
    // This is the one output in bailiff that embeds bytes an agent
    // chose. A length prefix makes the format safe to *parse*; a
    // terminal does not parse, it interprets, so an implementer can
    // emit CSI sequences that scroll away or overwrite the
    // `verification=failed` line sitting above its own output. The
    // whole verb exists so an operator can tell trustworthy output
    // from untrustworthy output, and that guarantee cannot survive a
    // renderer the untrusted party controls.
    if !allow_terminal && std::io::IsTerminal::is_terminal(&std::io::stdout()) {
        return Err(
            "refusing to write a dossier to a terminal: it embeds implementer stdout \
                    verbatim, and those bytes can rewrite what you see (including the \
                    verification lines). Redirect to a file or a pager, or pass \
                    --allow-terminal to accept that risk"
                .into(),
        );
    }
