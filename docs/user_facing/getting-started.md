...
1. **`pf_helper`** — `writ-agent-vm-pf-helper` from `agent-infra`. The
   ...
   *appended* to the file is refused, because `com.apple/*` holds
   ...
   cat <<'EOF' | sudo install -o root -g wheel -m 0644 /dev/stdin /etc/writ/agent-vm-pf-policy.json
   {
     "version": 1,
     "ipv4_pool": "192.168.0.0/16",
     "ipv6_pool": "fd83:b6f2:e57::/48",
   ...
   EOF
   ```
   `preflight` reports the loaded policy alongside the PF facts, and
   refuses (exit 1, naming the reason) if the file does not load. The
   helper and `writd` must be upgraded together: a `writd` from before
   protocol v2 still passes the bounds as arguments, which this helper
...
`agent_run_log_root` is a **top-level** key, not part of `agent_vm`. Every run
— host-spawned or inside a VM — writes its `stdout.log` / `stderr.log`
beneath it, and neither config section can own it: a daemon serving only
host-spawned runs has no `agent_vm` block at all.
